...
//    length SER IP port file_name hops
    public List<File> search(MessageObject msgObject, List<Neighbour> neighbours, InetAddress nodeAddress, Integer nodeUdpPort) {
        ...
        for (int i = 0; i < neighbours.size(); i++) {
            if (!(neighbours.get(i).getAddress().toString().equals(msgObject.getSenderIP())) && !(neighbours.get(i).getPort()==msgObject.getSenderPort())){
                UdpHelper.sendMessage(searchMsg, neighbours.get(i).getAddress(), neighbours.get(i).getPort());
            }
        }
    return null;}
}
